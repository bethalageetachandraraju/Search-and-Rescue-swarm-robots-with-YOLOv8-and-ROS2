...
class Yolo_subscriber(Node):
    ...
    def yolo_callback(self, data):
        ...
        for r in data.yolov8_inference:
            ...
            if class_name == "person":
                print("Human detected. Attraction behavior turned ON")
                yolo_subscriber.get_logger().info("Location Captured")
 #           yolo_subscriber.get_logger().info(f"{self.cnt} {class_name} : {top}, {left}, {bottom}, {right}")
            ...
            if class_name == "person":
                print("Human detected. Attraction behavior turned ON, assistive bots are coming")
